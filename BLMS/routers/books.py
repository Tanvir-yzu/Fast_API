...
def sort_books(
    ...
    if sort_by == "book_id":
        ...
        book_list.sort(key=lambda x: str(getattr(x, sort_by)), reverse=reverse)             
...
def create_book(book: Book):
    ...
    for existing_book in books:
        if str(existing_book.book_id) == str(book.book_id):
            raise HTTPException(status_code=409, detail=f"Book with ID {book.book_id} already exists")
            
    # Convert Pydantic model to dict (storing book_id as string for consistency with your old logic)
    new_book = book.model_dump()
    new_book["book_id"] = str(new_book.book_id)
